  ...
  const categories: SystemCategory[] = [
    {
      ...
      icon: <IconMoney className="w-10 h-10 text-emerald-600" />,
      products: [
        {
    ...
    },
    {
      ...
      icon: <IconLock className="w-10 h-10 text-red-500" />,
      products: [
        {
    ...
    },
    {
      ...
      icon: <IconCart className="w-10 h-10 text-purple-600" />,
      products: [
        {
    ...
    },
    {
      ...
      icon: <IconHeart className="w-10 h-10 text-rose-500" />,
      products: [
        {
    ...
    },
    {
      ...
      icon: <IconBook className="w-10 h-10 text-blue-500" />,
      products: [
        {
    ...
    },
    {
      ...
      icon: <IconTruck className="w-10 h-10 text-amber-500" />,
      products: [
        {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="container mx-auto px-4 py-24 text-center">
        <div className="inline-block mb-6 p-2 px-4 rounded-full bg-white border border-slate-200 text-xs font-bold uppercase tracking-wider text-accent flex items-center gap-2 mx-auto w-fit shadow-sm">
      ...
      <div className="container mx-auto px-4 pb-24 space-y-20">
        {categories.map((category, idx) => (
          <div key={idx} className="relative bg-white p-8 md:p-12 rounded-3xl border border-slate-200 shadow-xl shadow-slate-200/40">
            {/* Category Header */}
            <div className="flex flex-col md:flex-row items-center md:items-start gap-6 mb-12 border-b border-slate-100 pb-8">
              <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 shadow-sm">
                {category.icon}
              </div>
              <div className="text-center md:text-left">
                <h3 className="text-3xl font-bold text-slate-900 mb-2">{category.title}</h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {category.products.map((product, pIdx) => (
                <div key={pIdx} className="group flex flex-col bg-slate-50 border border-slate-200 rounded-2xl p-8 hover:border-accent hover:bg-white transition-all duration-300 hover:shadow-xl hover:shadow-accent/5">
                  <div className="mb-6">
                    <h4 className="text-2xl font-bold text-slate-900 mb-2 group-hover:text-accent transition-colors">
                      {product.title}
                    </h4>
                    <p className="text-slate-500 leading-relaxed mb-4 text-sm h-10">
                      {product.description}
                    </p>
                    <div className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm">
                        <span className="text-[10px] font-bold text-slate-400 uppercase block mb-2 tracking-wider">Recursos Inclusos:</span>
                        <ul className="space-y-2">
                          {product.features.map((feature, fIdx) => (
                            <li key={fIdx} className="flex items-center gap-2 text-slate-600 text-sm font-medium">
                  ...
                  <a 
                    ...
                    className="mt-auto w-full py-4 rounded-xl bg-white text-slate-700 font-bold border border-slate-300 hover:bg-accent hover:border-accent hover:text-white transition-all flex items-center justify-center gap-2 shadow-sm"
                  >
      ...
      <div className="container mx-auto px-4 pb-20">
        <div className="p-12 bg-gradient-to-r from-slate-900 to-slate-800 rounded-3xl border border-slate-700 text-center relative overflow-hidden shadow-2xl">
